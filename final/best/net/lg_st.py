...
import sys
sys.path.append('/home/dohwan/Noisy-ArcMix')

from aug import AugLayer, StatExLayer
...
class MobileFaceNet(nn.Module):
    def __init__(self,
        ...
        self.conv2 = ConvBlock(bottleneck_setting[-1][1], 768, 1, 1, 0)
        # 20(10), 4(2), 8(4)
        self.linear7 = ConvBlock(768, 768, (8, 20), 1, 0, dw=True, linear=True)
        
        self.linear1 = ConvBlock(768, 128, 1, 1, 0, linear=True)
    ...
    def forward(self, x):
        ...
        x = self.dw_conv1(x)
        x = self.blocks(x)
        x = self.conv2(x) # [64, 512, 8, 20]
        x = self.linear7(x) # [64, 512, 1, 1]
        x = self.linear1(x)
...
class TgramNet(nn.Module):
    def __init__(self, num_layer=6, mel_bins=128, win_len=1024, hop_len=512):
...
class TASTgramMFN(nn.Module):
    def __init__(self, num_classes, mode,
        ...
        self.mode = mode
        self.frequency_attention = Frequency_Attention(feature_dim=313)
    ...
    def forward(self, x_wav, x_mel, label, train=True):
        ...
        # x_mel_freq_att = self.frequency_attention(x_mel).unsqueeze(1) # [B, 1, 128, 313]

        stat = StatExLayer(0.5)
        x_mel_temp_att_, origin_X = stat(x_mel_temp_att)
        if self.bool:
            print("Large")
            print("statx")
            self.bool = False

        x = torch.cat((x_t, x_mel, x_mel_temp_att_), dim=1)
...
class Frequency_Attention(nn.Module):
    def __init__(self, feature_dim=313):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x1 = self.max_pool(x)  # (B, 128, 1)
        x2 = self.avg_pool(x)  # (B, 128, 1)
        
        feats = x1 + x2
        feats = feats.repeat(1, 1, self.feature_dim) #[B, 128, 313]
        
        refined_feats = self.sigmoid(feats) * x
        return refined_feats # [B, 128, 313]  

'''net = TASTgramMFN(num_classes=41, mode='noisy_arcmix', use_arcface=True, m=0.5).to('cuda')
x_wav = torch.randn(2, 1, 160000).to('cuda')  # Example input
x_mel = torch.randn(2, 1, 128, 313).to('cuda')  # Example mel spectrogram input
label = torch.randn(2).to('cuda')
print(sum(p.numel() for p in net.parameters()))
y = net(x_wav, x_mel, label)
a = 1 - 2'''